(function() {
	...
	Events.prototype = {
		...
		trigger: function(name) {
			...
			if (this._handlers[name]) {
				...
				var isArray = (AArray.test(handler) == 1);
	...
	var selectWordHandler = function(handlerName) {
		return function(e) {
			...
			var element = A.one(e.currentTarget._node);

			var word = Lang.trim(element.getData('word') || element.text());
	...
	var Collection = function(elements, instanceFactory) {
		...
		this.methods(['destroy', 'on', 'trigger']);
	};
	...
	var Box = function(config, parser, element) {
		...
		this.spellCheckerElement = A.one(element);
	...
	IncorrectWordsBox.prototype.bindEvents = function() {
		this.container.on(
			'click',
			'a',
			selectWordHandler.call(this, 'select.word')
		);

		this.on(
			'addWords',
			this.addWords.bind(this)
		);
	};

	IncorrectWordsBox.prototype.createBox = function() {
		var incorrectWords = this.config.incorrectWords;
		var position = incorrectWords.position;
		...
		if (A.isFunction(position)) {
			position.call(this.spellCheckerElement, this.container);
		}
		else {
			this.container.appendTo(incorrectWords.container);
		}
	};
	...
	IncorrectWordsBox.prototype.addWords = function(words) {
		words = A.grep(
			words,
			function(el, index) {
				return (index === AArray.indexOf(words, el));
			}
		);
	...
	IncorrectWordsInline.prototype.addWords = function(words) {
		...
		var highlighted = this.parser.highlightWords(words, element);
	...
	var SuggestBox = function(config, element) {
		...
		if (config.suggestBox.appendTo) {
			this.body = A.one(config.suggestBox.appendTo);
		}
		else {
	...
	SuggestBox.prototype.bindEvents = function() {
		...
		var clickPlugin = click + '.' + pluginName;
		...
		container.on(
			clickPlugin,
			'.ignore-all',
			this.handler('ignore.all')
		);

		container.on(
			clickPlugin,
			'.ignore-forever',
			this.handler('ignore.forever')
		);

		container.delegate(
			click,
			selectWordHandler.call(this, 'ignore.word'),
			'.ignore-word'
		);
		...
		if (this.element.nodeName === 'BODY') {
			var onWindowClick = A.bind(this.onWindowClick, this);
			var parentNode = A.one(this.element.parentNode);

			parentNode.delegate(
				click,
				onWindowClick,
				parentNode.attr('localName')
			);
		}
	};
	...
	SuggestBox.prototype.showSuggestedWords = function(getWords, word, wordElement) {
		...
		getWords(
			word,
			this.onGetWords.bind(this)
		);
	};

	SuggestBox.prototype.loading = function(show) {
		var loadingMessage = show ? this.loadingMsg.clone() : '';

		this.words.html(loadingMessage);
	...
	SuggestBox.prototype.position = function() {
		...
		var boxOffset = config.suggestBox.offset;
		...
		if (config.suggestBox.position === 'below') {
			top = positionBelow;
	...
	/*WebService.prototype.makeRequest = function(config) {
		var defaultConfig = A.aggregate({}, this.defaultConfig);
	...
	SpellChecker.prototype.setupIncorrectWords = function() {
		...
		this.incorrectWords = new Collection(
			elements,
			function(element) {
	...
	SpellChecker.prototype.bindEvents = function() {
		this.on(
			'check.fail',
			this.onCheckFail.bind(this)
		);

		this.suggestBox.on(
			'ignore.word',
			this.onIgnoreWord.bind(this)
		);

		this.suggestBox.on(
			'select.word',
			this.onSelectWord.bind(this)
		);

		this.incorrectWords.instances[0].on(
			'select.word',
			this.onIncorrectWordSelect.bind(this)
		);
	};
	...
	SpellChecker.prototype.getSuggestions = function(word, callback) {
		var suggestBox = this.suggestBox;

		if (suggestBox) {
			suggestBox.loading(true);
		}
	...
	SpellChecker.prototype.onCheckWords = function(callback) {
		return function(data) {
			var incorrectWords = new Array(data);
